...
public class Bandit_Warrior : MonoBehaviour
{
    ...
    public void follow()
    {
        ...
        if (player.position.x > leftRange.position.x && player.position.x < rightRange.position.x
            && Math.Abs(transform.position.y - player.position.y) <= 15)
        {
            ...
            {
                p = new Vector3(player.position.x - 6, transform.position.y, transform.position.z);
            }
            else
            {
            ...
            if (transform.position.x != p.x)
                animator.Play("run");

            transform.position = Vector3.MoveTowards(transform.position, p, 20 * Time.deltaTime);

            if (Math.Abs(transform.position.x - player.position.x) <= 6)
            {
                if (Time.time > attackTime)
                {
                    meleeAttack();
                    attackTime = Time.time + delayTimeAttack;
                }
            }
        }
