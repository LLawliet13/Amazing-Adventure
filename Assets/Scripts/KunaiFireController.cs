...
public class KunaiFireController : MonoBehaviour
{
    ...
    public GameObject AutoDetect()
    {
        ...
        List<GameObject> insideCameraEnemies = new List<GameObject>();
        foreach (var enemy in enemies)
        {
            ...
            if (x >= left && x <= right &&
                y >= bottom && y <= top)
            {
                insideCameraEnemies.Add(enemy);
            }
        }
        if (insideCameraEnemies.Count == 0) return null;
        float minDistance = 9999999999;
        GameObject nearestEnemy = null;
        foreach (var enemy in insideCameraEnemies)
        {
            float currentDistance = Vector2.Distance(enemy.transform.position, mc.position);
