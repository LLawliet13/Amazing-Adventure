...
public class Sasuke : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        LoadSkill();
        GateFinal.SetActive(false);

    }
    public GameObject NPCKakashi;
    public GameObject GateFinal;
    public Transform NPCPlaceAfterBossDie;
    private void OnDisable()
    {
        if (transform.GetComponent<EnemyStats>().getCurrentHP() <= 0)
        {
            GameObject npcKakashiTemp = GameObject.FindGameObjectWithTag("NPC");
            if (npcKakashiTemp != null) Destroy(npcKakashiTemp);
            GameObject npcKakashi = Instantiate(NPCKakashi, NPCPlaceAfterBossDie.position, Quaternion.identity);
            npcKakashi.GetComponent<NPCKakashi>().setDialogName("kakasi_say_end_game");
            GameObject player = GameObject.FindGameObjectWithTag("Player");
            Vector3 newPos = new Vector3(NPCPlaceAfterBossDie.position.x - 10, NPCPlaceAfterBossDie.position.y, NPCPlaceAfterBossDie.position.z);
            GateFinal.SetActive(true);
            if (player != null)
    ...
    static float timeToNextUseLighting = 0;
    public void skill_lighting()
    {


        if (timeToNextUseLighting < Time.time)
        {
            animator.Play("skill");
            timeToNextUseLighting = Time.time + 30;
            return;
        }
    }
    static float timeToNextUseSkillFire = 0;
    public void skill_fire()
    {


        if (timeToNextUseSkillFire < Time.time)
        {
            animator.Play("skill_fire");
            timeToNextUseSkillFire = Time.time + 10;
            return;
        }
    }
    static float timeToNextUseSkillFireDragon = 0;
    public void skill_fire_dragon()
    {


        if (timeToNextUseSkillFireDragon < Time.time)
        {
            animator.Play("skill_fire_dragon");
            timeToNextUseSkillFireDragon = Time.time + 20;
            return;
        }
    }
    ...
    private void LoadSkill()
    {
        ...
        skillList.Add("normal_attack",
           new Skill("normal_attack", 1.5f, 3, 3, false, false, 1.5f,
           new Action(normal_attack)
           ));
        skillList.Add("jump_attack",
            new Skill("jump_attack", 1.5f, 3, 4, true, false, 1.5f,
            new Action(jump_attack)
            ));
        skillList.Add("skill_lighting",
            new Skill("skill_lighting", 40, 10, 8, false, false, 1,
            new Action(skill_lighting)
            ));
        skillList.Add("skill_fire",
            new Skill("skill_fire", 20, 3, 1, true, false, 1,
            new Action(skill_fire)
            ));
        skillList.Add("skill_fire_dragon",
            new Skill("skill_fire_dragon", 30, 500, 10, false, false, 1,
            new Action(skill_fire_dragon)
            ));
    ...
    public string skill_selected()
    {
        return skillNameSelected;
    }
    public bool DetectRestriction()
    {
        try
        {
            if (transform.position.x < leftRange.position.x || transform.position.x > rightRange.position.x ||
                transform.position.y < downRange.position.y || transform.position.y > upRange.position.y)
            {
                transform.position = startLocation.position;
                return true;
            }
            if (player.position.x < leftRange.position.x || player.position.x > rightRange.position.x ||
                player.position.y < downRange.position.y || player.position.y > upRange.position.y)
            {
                transform.position = startLocation.position;
                return true;
            }
