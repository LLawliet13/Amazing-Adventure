...
public class Cuuvi : MonoBehaviour
{
    ...
    public Transform startLocation;//nen de ngay tren 1 ground
    ...
    public Transform BantayPrefab;
    private float timeToNextUseBantay = 0;
    public void thothuatbantay()
    {
        if (Time.time > timeToNextUseBantay)
        {

            animator.Play("thothuat_bantay");
            timeToNextUseBantay = Time.time + 5;
            Instantiate(BantayPrefab, player.transform.position, Quaternion.identity);
        }

    }
    float timeToNextUseKunai = 0;
    public void throw_cuuviFire()
    {

        animator.Play("throw_cuuvifire");
        ...
        {
            //Debug.LogError("Cuu vi fire");
            kunai.GetComponent<KunaiFireController>().shoot("cuuviFire");
    ...
    private void LoadSkill()
    {
        ...
        skillList.Add("thothuatbantay",
            new Skill("thothuatbantay", 40, 500, 500, false, false, 1.5f,
            new Action(thothuatbantay)
            ));
        skillList.Add("throw_cuuviFire",
            new Skill("throw_cuuviFire", 20, 500, 500, false, false, 1.5f,
            new Action(throw_cuuviFire)
            ));
    ...
    private void OnDisable()
    {
        if (transform.GetComponent<EnemyStats>().getCurrentHP() <= 0)
        {
            ...
            GameObject npcKakashi = Instantiate(NPCKakashi, NPCPlaceAfterBossDie.position, transform.rotation);
            npcKakashi.GetComponent<NPCKakashi>().setDialogName("kakasi_say_end_map1");
